...
# Ссылка на сайт - ЗАМЕНИТЕ ПОСЛЕ ДЕПЛОЯ
SITE_URL = "https://darkdonate.netlify.app"
...
# ТОВАРЫ
PRODUCTS = {
    "kit_plus": {"name": "⚡ КИТ PLUS", "price": 9, "stars": 10, "desc": "Полуалмазная броня, меч острота I"},
    "kit_rare": {"name": "💎 КИТ РЕДКИЙ", "price": 19, "stars": 21, "desc": "Алмазная броня, меч острота II"},
...
def process_card_payment(message, product, original_msg):
    ...
    bot.send_message(
        ...
        f"📝 <b>В комментарии укажите:</b> <code>{nick}</code>\n\n"
...
def how_to_buy(call):
    ...
    bot.edit_message_text(
        ...
        f"5️⃣ В комментарии укажите никнейм\n"
        f"6️⃣ Нажмите \"ПОДТВЕРДИТЬ ОПЛАТУ\"\n\n"
        f"📞 Поддержка: @DarkYo_offical",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        parse_mode='HTML',
